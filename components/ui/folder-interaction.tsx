...
export interface FolderShotItem {
  shotNumber: number;
  timeRange: string;
  script: string;
  action?: string;
...
export interface FolderInteractionProps {
...
export function FolderInteraction({
  ...
  return (
    <div className={`w-full flex flex-col justify-center items-center select-none ${className}`}>
      {/* Folder Header Label */}
      <div className="mb-2 text-center cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
        <span className="text-xs font-mono font-bold text-cyan-400 uppercase tracking-wider">
          📁 {title} ({duration}) — {isOpen ? "Tap to close folder" : "Tap folder to expand shots"}
        </span>
      </div>

      <div
        onClick={() => setIsOpen(!isOpen)}
        className="w-80 h-52 relative wrapper cursor-pointer transition-transform hover:scale-[1.02]"
      >
        <div
          ...
          style={{
            background: "#18151B",
            boxShadow:
              "0px 0px 15.7px 16px rgba(79, 73, 85, 0.30) inset",
            borderRadius: 10,
          }}
        >
          {[
            ...
            const shot = hasShots ? shots[page.shotIndex] || shots[i % shots.length] : null;
            return (
              <motion.div
                key={i}
                ...
                className={`absolute top-2 w-36 h-fit rounded-xl ${page.className}`}
                onClick={(e) => {
                  if (isOpen && onSelectShot && shot) {
                    e.stopPropagation();
                    onSelectShot(page.shotIndex);
                  }
                }}
              >
                {shot ? (
                  <ShotPageCard shot={shot} />
                ) : (
                  <DefaultPageCard />
                )}
              </motion.div>
        ...
        >
          ...
          >
            ...
            <defs>
              ...
              <linearGradient id="paint0_linear_1_106" x1="114.7" y1="0.700104" x2="114.7" y2="121.7" gradientUnits="userSpaceOnUse">
                <stop stopColor="#2D2535" />
                <stop offset="1" stopColor="#2A2A2A" />
              </linearGradient>
              <linearGradient id="paint1_linear_1_106" x1="114.7" y1="0.700104" x2="114.7" y2="121.7" gradientUnits="userSpaceOnUse">
...
const ShotPageCard = ({ shot }: { shot: FolderShotItem }) => (
  <div className="w-full h-full bg-gradient-to-b from-[#1e2338] to-[#0f1322] text-white rounded-xl shadow-xl p-2.5 sm:p-3 border border-cyan-500/30 flex flex-col justify-between overflow-hidden">
    <div>
      <div className="flex justify-between items-center mb-1">
        <span className="text-[10px] font-mono font-bold text-cyan-400 bg-cyan-950/70 px-1.5 py-0.5 rounded">
      ...
      <p className="text-[10px] text-slate-200 line-clamp-3 leading-tight font-medium">
        &ldquo;{shot.script}&rdquo;
      </p>
    </div>
    {shot.action && (
      <p className="text-[9px] text-emerald-400 line-clamp-1 mt-1 font-mono">
        🎬 {shot.action}
      </p>
    )}
  </div>
